<form class="form-horizontal" id="form" role="form" action="{{ route('info_sistema.cadastrado') }}" method="POST" enctype="multipart/form-data" data-parsley-validate>
    @csrf
    <div class="groups-two" style="flex-direction: column"> 
		<span for="title" class="control-label">Title</span>
		<input type="title" class="inputs" id="title" name="title" placeholder="Insira o titulo aqui...">
    </div>
    <div class="groups-two" style="flex-direction: column"> 
		<span for="legenda" class="control-label">Legenda</span>
		<input type="legenda" class="inputs" id="legenda" name="legenda" placeholder="Insira a legenda aqui...">
	</div>
	<div class="form-banner">
		<span for="banner" class="control-label">Imagem</span>
		<span>Obs: Tamanho recomendado: largura: 2000px, Altura: 506px.</span>
		<div class="input-group">
			<div class="custom-file"> 
				<input type="file" accept="image/*" name="img" class="inputs" id="img" onchange="previewImagem()">
				<label class="custom-file-label" for="inputGroupFile04">Escolha uma imagem</label>
			</div>
		</div>
    </div>
	
	<img style="width: 100%; height: 100%;margin: 0.5rem 0;" name="img">
